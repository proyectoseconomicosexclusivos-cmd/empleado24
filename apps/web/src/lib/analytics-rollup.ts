...
export async function refreshAnalyticsDaily(date = new Date()) {
  ...
  const [
    ...
    admin
      ...
      .select('event_name,visitor_id,anonymous_id,metadata')
    ...
    admin
      .from('subscriptions')
      .select('state,plan_key,trial_ends_at')
      .in('state', ['active', 'past_due', 'incomplete', 'canceled', 'canceling', 'trialing']),
  ]);
  const error = eventsError ?? companiesError ?? invoicesError ?? subscriptionsError;
  if (error) throw error;
  const items = (events ?? []) as Array<{
    event_name: string;
  ...
  const visitors = new Set(
    items
      .filter(
        (event) =>
          ['landing_view', 'page_view', 'pricing_view'].includes(event.event_name) &&
          event.metadata?.action !== 'click' &&
          event.metadata?.action !== 'scroll_depth',
      )
      .map((event) => event.visitor_id ?? event.anonymous_id)
      .filter(Boolean),
  ).size;
  const current = (subscriptions ?? []) as Array<{
    state: string;
  ...
  const result = await admin.from('analytics_daily').upsert(
    {
      ...
      revenue_cents: (paidInvoices ?? []).reduce(
        (sum: number, invoice: { amount_paid_cents?: number | null }) =>
          sum + Number(invoice.amount_paid_cents ?? 0),
        0,
      ),
      mrr_cents: current
        .filter((subscription) => subscription.state === 'active')
        .reduce((sum, subscription) => sum + (planPrice[subscription.plan_key ?? ''] ?? 0), 0),
