...
function countSince(rows: Array<{ sent_at: string; direction: string }>, since: Date, direction?: string) {
  return rows.filter((row) => new Date(row.sent_at) >= since && (!direction || row.direction === direction)).length;
}

function statusLabel(status: string, metadata: Record<string, unknown> | null) {
  const label = typeof metadata?.label === 'string' ? metadata.label : null;
  if (label) return label;
  return status === 'open' ? 'En conversación' : status === 'waiting' ? 'Esperando cliente' : status === 'escalated' ? 'Necesita atención' : 'Cerrada';
}
...
export default async function WhatsAppPage({ searchParams }: { searchParams: Promise<{ connected?: string }> }) {
  ...
  const [{ data: employee }, integrations, { data: conversations }, { data: messages }] = await Promise.all([
    ...
    supabase.from('whatsapp_conversations').select('id,status,customer_name,customer_phone,metadata,last_customer_message_at,first_response_at,updated_at').eq('company_id', company.id).order('updated_at', { ascending: false }).limit(100),
  ...
  const publicConfig = connected && typeof connected.public_config === 'object' && connected.public_config !== null && !Array.isArray(connected.public_config)
    ? connected.public_config as Record<string, unknown>
    : {};
  const phoneNumber = typeof publicConfig.phone_number === 'string' ? publicConfig.phone_number : null;
  const highPriority = rows.filter((row) => ['high', 'urgent'].includes(typeof row.metadata?.priority === 'string' ? row.metadata.priority : '')).length;
  ...
  return <main className="mx-auto max-w-6xl px-5 py-10 md:px-10 md:py-14"><header className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">Tu empleado</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.06em]">💬 {employee.name}</h1><p className="mt-3 text-[var(--muted)]">Tu Empleado WhatsApp IA está trabajando para {company.name}.</p></div><Link href="/app/integraciones/whatsapp_meta" className="inline-flex items-center gap-2 rounded-full border border-[var(--line)] px-4 py-2 text-sm font-medium">{connected ? 'Revisar WhatsApp Business' : 'Conectar WhatsApp Business'} <ArrowRight size={14}/></Link></header>
    {(query.connected === '1' || connected) && <div className="mt-8 rounded-2xl bg-[#e9ffcf] p-4 text-sm text-[#486500] dark:bg-[#293500] dark:text-[#d5f899]"><CheckCircle2 className="mr-2 inline" size={17}/>Todo correcto. Tu WhatsApp Business está conectado y pertenece solo a tu empresa.{phoneNumber ? ` Número: ${phoneNumber}.` : ''}</div>}
    <section className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Fact icon={CheckCircle2} label="Estado" value={connected ? 'Conectado' : 'Por conectar'}/><Fact icon={MessageCircle} label="Mensajes hoy" value={String(countSince(messagesRows, today))}/><Fact icon={MessageCircle} label="Mensajes esta semana" value={String(countSince(messagesRows, week))}/><Fact icon={MessageCircle} label="Mensajes este mes" value={String(countSince(messagesRows, month))}/><Fact icon={MessageCircle} label="Conversaciones abiertas" value={String(rows.filter((row) => ['open','waiting'].includes(row.status)).length)}/><Fact icon={CheckCircle2} label="Conversaciones cerradas" value={String(rows.filter((row) => row.status === 'closed').length)}/><Fact icon={Users} label="Clientes prioritarios" value={String(highPriority)}/><Fact icon={Clock3} label="Respuesta media" value={averageResponse(rows)}/></section>
    <section className="mt-10 rounded-3xl border border-[var(--line)] p-6"><h2 className="text-xl font-semibold">Conversaciones recientes</h2>{rows.length ? <ul className="mt-5 divide-y divide-[var(--line)]">{rows.slice(0,12).map((row) => <li key={row.id} className="flex items-center justify-between gap-4 py-4"><div><p className="font-medium">{row.customer_name || 'Cliente de WhatsApp'}</p><p className="mt-1 text-sm text-[var(--muted)]">{row.customer_phone} · Última actividad: {new Date(row.updated_at).toLocaleString('es-ES')}</p></div><span className="rounded-full bg-black/5 px-3 py-1 text-xs text-[var(--muted)] dark:bg-white/5">{statusLabel(row.status, row.metadata)}</span></li>)}</ul> : <p className="mt-5 text-sm leading-6 text-[var(--muted)]">Cuando un cliente escriba por WhatsApp, tu empleado atenderá la conversación aquí.</p>}</section>
  </main>;
}
