  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
      <aside className="relative hidden overflow-hidden bg-[#111315] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="noise" />
        <Link href="/" className="relative text-lg font-bold tracking-[-.07em]">EMPLEADO<span className="text-[#ccff00]">24</span></Link>
        <div className="relative max-w-xl">
          <span className="grid h-16 w-16 place-items-center rounded-3xl bg-[#ccff00] text-[#111315]"><Headphones size={27} /></span>
          <h2 className="mt-10 text-5xl font-semibold tracking-[-.065em]">Hoy puede ser su primer día.</h2>
          <ul className="mt-8 grid gap-4 text-sm text-white/65">
            {['Aprenderá cómo funciona tu empresa', 'Atenderá con tus palabras', 'Te pedirá ayuda cuando la necesite'].map((item) => <li key={item} className="flex gap-3"><Check size={17} className="text-[#ccff00]" />{item}</li>)}
          </ul>
        </div>
      ...
      <section className="grid place-items-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <form action={submit} className="mt-9">
            ...
            <label className="mt-5 block text-sm font-medium" htmlFor="email">Tu email de trabajo</label>
            <input id="email" name="email" type="email" autoComplete="email" required placeholder="tu@empresa.com" className="input mt-2" />
            <label className="mt-5 block text-sm font-medium" htmlFor="password">Crea una contraseña</label>
            ...
            <button disabled={pending} className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#ccff00] p-3 font-semibold text-[#111315] transition hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-60">
              {pending ? 'Preparando la bienvenida…' : 'Empezar la contratación'} <ArrowRight size={16} aria-hidden="true" />
            </button>
            {message && <p role={isError ? 'alert' : 'status'} className={`mt-5 rounded-xl p-4 text-sm ${isError ? 'bg-[#fff0eb] text-[#7b3c2b] dark:bg-[#3c211a] dark:text-[#ffc9b8]' : 'bg-[#efffcf] text-[#486500] dark:bg-[#293500] dark:text-[#d5f899]'}`}>{message}</p>}
          </form>
          <p className="mt-7 text-sm text-[var(--muted)]">¿Ya forma parte de tu equipo? <Link href="/login" className="font-medium text-[var(--fg)]">Entrar en tu empresa</Link></p>
        </div>
      </section>
    </main>
  );
