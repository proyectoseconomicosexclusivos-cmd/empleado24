...
export default async function Customer360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: identities }, { data: memories }, { data: events }, { data: tasks }, { data: opportunities }, { data: technicalProjects }] = await Promise.all([
    ...
    admin.from('technical_projects').select('id,title,status,updated_at').eq('customer_id', id).order('updated_at', { ascending: false }).limit(30),
  ]);
